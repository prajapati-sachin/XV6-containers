...
//Initialising scheduler and memory log
// memory_log = 0;
// scheduler_log = 0;
...
void init_containerlist(void){
	initlock(&container_list.lock, "container_list_lock");
...
int create_container(){
	...
	int i=-1;
	for(i=1;i<NPROC;i++){
		if(container_list.alloc[i]==0) break;
	}
...
int join_container(int i){
	struct proc *p = myproc();
	
	acquire(&ptable.lock);
	if(p->container_id!=0 || i==0){
		release(&ptable.lock);
		return -1;
	}
	// if(i==1){
	...
	int j;
	for(j=0;j<NPROC;j++){
		if(container_list.containers[i].pids[j]==-1)
			break;
	}
	if(j==NPROC){
		release(&container_list.lock);
	...
	container_list.containers[i].pids[j]=p->pid;
	release(&container_list.lock);
...
int leave_container(){
	...
	int j;
	for(j=0;j<NPROC;j++){
		if(container_list.containers[my_cnid].pids[j]==my_pid)
			break;
	}
	if(j==NPROC){
		release(&container_list.lock);
	...
	container_list.containers[my_cnid].pids[j]=-1;
	release(&container_list.lock);
